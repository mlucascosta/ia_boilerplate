...
```bash
INIT=$(node ".agents/bin/gsd-tools.cjs" init phase-op "${PHASE_ARG}")
...
node ".agents/bin/gsd-tools.cjs" roadmap get-phase "$phase_number"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node ".agents/bin/gsd-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(node ".agents/bin/gsd-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(node ".agents/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
