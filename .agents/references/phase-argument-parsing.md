...
```bash
PHASE_INFO=$(node ".agents/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ".agents/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node ".agents/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
