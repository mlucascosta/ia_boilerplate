...
AGENT_SKILLS_AUDITOR=$(node ".agents/bin/gsd-tools.cjs" agent-skills gsd-nyquist-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(node ".agents/bin/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node ".agents/bin/gsd-tools.cjs" config-get workflow.nyquist_validation --raw)
```
...
**State B (create):**
1. Read template from `.agents/templates/VALIDATION.md`
...
node ".agents/bin/gsd-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
