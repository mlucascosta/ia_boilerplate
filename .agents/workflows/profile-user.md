...
Key references:
- @.agents/references/ui-brand.md (display patterns)
- @.agents/agents/gsd-user-profiler.md (profiler agent definition)
- @.agents/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH=".agents/USER-PROFILE.md"
...
```bash
cp ".agents/USER-PROFILE.md" ".agents/USER-PROFILE.backup.md"
```
...
✓ Stores profile at .agents/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node .agents/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node .agents/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `.agents/references/user-profiling.md`
...
Reference: @.agents/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node .agents/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node .agents/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node .agents/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to .agents/USER-PROFILE.md"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at .agents/USER-PROFILE.md" and jump to step 10.
...
```bash
node .agents/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node .agents/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node .agents/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```
...
Your profile:    .agents/USER-PROFILE.md
```
