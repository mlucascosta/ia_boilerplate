...
<required_reading>
@.agents/references/verification-patterns.md
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node ".agents/bin/gsd-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node ".agents/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node ".agents/bin/gsd-tools.cjs" verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node ".agents/bin/gsd-tools.cjs" verify key-links "$plan")
...
See .agents/templates/verification-report.md for complete template.
</step>
