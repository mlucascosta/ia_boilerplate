...
```bash
INIT=$(node ".agents/bin/gsd-tools.cjs" init todos)
...
```bash
slug=$(node ".agents/bin/gsd-tools.cjs" generate-slug "$title" --raw)
```
...
```bash
node ".agents/bin/gsd-tools.cjs" commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
