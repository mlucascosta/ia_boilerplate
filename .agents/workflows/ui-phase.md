...
AGENT_SKILLS_UI=$(node ".agents/bin/gsd-tools.cjs" agent-skills gsd-ui-researcher 2>/dev/null)
AGENT_SKILLS_UI_CHECKER=$(node ".agents/bin/gsd-tools.cjs" agent-skills gsd-ui-checker 2>/dev/null)
```
...
```bash
UI_RESEARCHER_MODEL=$(node ".agents/bin/gsd-tools.cjs" resolve-model gsd-ui-researcher --raw)
UI_CHECKER_MODEL=$(node ".agents/bin/gsd-tools.cjs" resolve-model gsd-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(node ".agents/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
```
...
Template: .agents/templates/UI-SPEC.md
</output>
...
```bash
node ".agents/bin/gsd-tools.cjs" commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
