...
```bash
node ".agents/bin/gsd-tools.cjs" phases list --type summaries --raw
...
```bash
node .agents/bin/gsd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See .agents/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node ".agents/bin/gsd-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@.agents/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `.agents/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `.agents/templates/summary.md`.
...
node ".agents/bin/gsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node ".agents/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node ".agents/bin/gsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
